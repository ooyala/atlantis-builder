...
sudo docker tag $ID $REGISTRY_HOST/builder/precise64-java1.7-$VERSION

if [[ "$1" == "-p" ]]; then
  echo "Pushing..."
  sudo docker push $REGISTRY_HOST/builder/precise64-java1.7-$VERSION
  echo "Done."
else
  echo "Done. To push the new image:"
  echo "sudo docker push $REGISTRY_HOST/builder/precise64-java1.7-$VERSION"
fi
