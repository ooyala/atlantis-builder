...
sudo docker tag $ID $REGISTRY_HOST/builder/precise64-go1.2-$VERSION

if [[ "$1" == "-p" ]]; then
  echo "Pushing..."
  sudo sudo docker push $REGISTRY_HOST/builder/precise64-go1.2-$VERSION
  echo "Done."
else
  echo "Done. To push the new image:"
  echo "sudo docker push $REGISTRY_HOST/builder/precise64-go1.2-$VERSION"
fi
